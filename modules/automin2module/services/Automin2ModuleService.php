...
class Automin2ModuleService extends Component
{
    ...
    private function _init_settings() {
        $base_path = realpath(CRAFT_BASE_PATH.'web'.'/');
        $settings = [
            'autominEnabled' => true,
            'autominCachingEnabled' => false,
            'autominMinifyEnabled' => true,
            'autominPublicRoot' => $base_path,
            'autominCachePath' => $base_path.'cache/',
            'autominCacheURL' => '/cache/',
            'autominSCSSIncludePaths' => $base_path.'assets/css/'
        ];
    ...
    private function _process_markup($markup, $markup_type, $markup_attrs) {
        ...
        $cache_key = Automin2Module::getInstance()->automin2ModuleCacheService->get_cache_key($markup, $extension);

        // Fetch and validate cache, if caching is enabled
        if ($this->getSetting('autominCachingEnabled')) {
            $cache_filename = Automin2Module::getInstance()->automin2ModuleCacheService->fetch_cache(
                $cache_key,
                $markup,
                $last_modified
            );
        ...
        // Cache output
        $cache_result = Automin2Module::getInstance()->automin2ModuleCacheService->write_cache($cache_key, $combined_file_data);
        ...
        try {
            ...
            switch($markup_type) {
                ...
                case self::MARKUP_TYPE_LESS:
                    ...
                    // Compile with LESS
                    require_once(self::CRAFT_MODULES_PATH.'automin2module/vendor/lessphp/lessc.inc.php');
                    $less_obj = new \lessc();
                    $code = $less_obj->parse($code);

                    if ($this->settings['autominMinifyEnabled']) {
                        // Compress CSS
                        require_once(self::CRAFT_MODULES_PATH.'automin2module/vendor/class.minify_css_compressor.php');
                ...
                case self::MARKUP_TYPE_SCSS:
                    ...
                    // Compile with SCSS
                    require_once(self::CRAFT_MODULES_PATH.'automin2module/vendor/scssphp/scss.inc.php');
                ...
                case self::MARKUP_TYPE_JS:
                    ...
                    if ($this->settings['autominMinifyEnabled']) {
                        // Compile JS
                        require_once(self::CRAFT_MODULES_PATH.'automin2module/vendor/class.jsmin.php');
